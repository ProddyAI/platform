 ...
 */
function _convexTypeToJsonType(convexType) {
    const typeMap = {
        string: "string",
        number: "number",
        float64: "number",
 ...
 */
export function extractTableInfo(tableName, tableDefinition, config) {
 ...
 */
function generateQueryTool(table, handler, tableDescription) {
 ...
 */
function generateCountTool(table, handler) {
    const fieldNames = table.fields.map((f) => f.name);
    return {
 ...
 */
function generateAggregateTool(table, handler) {
 ...
 */
function generateSearchTool(table, handler) {
 ...
 */
function generateGetByIdTool(table, handler) {
    return {
 ...
 */
export function generateToolsFromSchema(options) {
    const { tables, allowedTables, excludeFields: _excludeFields, tableDescriptions, handlers, } = options;
    const tools = [];
    // Filter to only allowed tables
    const filteredTables = tables.filter((t) => allowedTables.includes(t.name));
    for (const table of filteredTables) {
        // Query tool (always)
        tools.push(generateQueryTool(table, handlers.query, tableDescriptions?.[table.name]));
        // Count tool (always)
