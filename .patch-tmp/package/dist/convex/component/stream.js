...
import { requireConversationExternalId, requireStreamExternalId, } from "./access";
...
const streamPartValidator = v.object({
    type: v.union(v.literal("text-delta"), v.literal("tool-call"), v.literal("tool-result"), v.literal("error")),
    // For text-delta
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingStream = await ctx.db
            ...
            .withIndex("by_conversation_status", (q) => q.eq("conversationId", args.conversationId).eq("status", "streaming"))
            .first();
        if (existingStream) {
            ...
            await ctx.scheduler.runAfter(CLEANUP_DELAY_MS, internal.stream.cleanupStream, { streamId: existingStream._id });
        }
        const now = Date.now();
        // Create the stream
        ...
        const timeoutFnId = await ctx.scheduler.runAfter(TIMEOUT_INTERVAL_MS, internal.stream.timeoutStream, { streamId });
...
export const finish = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.scheduler.runAfter(CLEANUP_DELAY_MS, internal.stream.cleanupStream, { streamId: args.streamId });
        return null;
    },
});
/**
...
export const abortByConversation = mutation({
    ...
    returns: v.boolean(),
    handler: async (ctx, args) => {
        return await abortStreamByConversationId(ctx, args.conversationId, args.reason);
    },
});
/**
...
export const abortForExternalId = mutation({
    ...
    returns: v.boolean(),
    handler: async (ctx, args) => {
        await requireConversationExternalId(ctx, args.conversationId, args.externalId);
        return await abortStreamByConversationId(ctx, args.conversationId, args.reason);
    },
});
/**
...
export const timeoutStream = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (timeSinceHeartbeat < TIMEOUT_INTERVAL_MS) {
            // Reschedule timeout
            const timeoutFnId = await ctx.scheduler.runAfter(TIMEOUT_INTERVAL_MS - timeSinceHeartbeat, internal.stream.timeoutStream, { streamId: args.streamId });
...
export const cleanupStream = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (remainingDeltas) {
            ...
            await ctx.scheduler.runAfter(1000, // 1 second delay between batches
            internal.stream.cleanupStream, { streamId: args.streamId });
            return null;
        }
 ...
 */
async function deleteStreamDeltas(ctx, streamId) {
...
async function getStreamState(ctx, conversationId) {
    // First, try to find an active streaming stream
    const activeStream = await ctx.db
        .query("streamingMessages")
        .withIndex("by_conversation_status", (q) => q.eq("conversationId", conversationId).eq("status", "streaming"))
        .first();
    if (activeStream) {
        return {
...
async function listStreamDeltas(ctx, streamId, cursor) {
    // Ensure cursor is non-negative for defensive programming
    const safeCursor = Math.max(0, cursor);
    const deltas = await ctx.db
        .query("streamDeltas")
        .withIndex("by_stream_cursor", (q) => q.eq("streamId", streamId).gte("start", safeCursor))
...
async function abortStreamByConversationId(ctx, conversationId, reason) {
    const stream = await ctx.db
        .query("streamingMessages")
        .withIndex("by_conversation_status", (q) => q.eq("conversationId", conversationId).eq("status", "streaming"))
        .first();
