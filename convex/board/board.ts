	...
	handler: async (
		...
		if (boardUserId && channel?.workspaceId) {
			await ctx.scheduler.runAfter(
				0,
				internal.billing.usageTracking.recordBoardCreated,
				{
		...
		if (args.assignees && args.assignees.length > 0) {
			try {
				...
				for (const assigneeId of args.assignees) {
					...
					await ctx.scheduler.runAfter(
						0,
						internal.notify.emailActions.sendCardAssignmentEmail,
						{
							assigneeId,
							cardId,
...
export const updateCard = mutation({
	...
	handler: async (
		...
		await ctx.scheduler.runAfter(0, api.search.ragchat.autoIndexCard, {
			cardId,
		});
				...
				if (auth) {
					...
					if (creator) {
						for (const assigneeId of validatedAssignees) {
							...
							await ctx.scheduler.runAfter(
								0,
								internal.notify.emailActions.sendIssueAssignmentEmail,
								{
									assigneeId,
									issueId,
									assignerId: creator._id,
