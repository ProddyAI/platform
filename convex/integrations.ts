...
// Get active connected app names for assistant (user-specific)
export const getMyActiveConnectedAppNames = query({
	args: {
		workspaceId: v.id("workspaces"),
	},
	handler: async (ctx, args) => {
...
export const adminDeleteConnectedAccount = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (
			!callerMember ||
			(callerMember.role !== "admin" && callerMember.role !== "owner")
		) {
