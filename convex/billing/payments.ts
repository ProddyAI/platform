...
const calculateUnusedPeriodRefundAmount = async (
	...
	try {
		const usage = await ctx.runQuery(
			internal.billing.payments.getBillingConsumptionForPeriod,
			{
				workspaceId,
				periodStart,
...
const getWorkspacePortalBillingContext = async (
	...
	const workspace: Doc<"workspaces"> | null = await ctx.runQuery(
		internal.workspace.workspaces.getWorkspaceByIdInternal,
		{
			id: workspaceId,
		}
	);
...
const createRecoveryCheckoutUrl = async (
	...
	const billingContext: {
		billableMemberCount: number;
	} = await ctx.runQuery(internal.billing.payments.getBillingSeatContext, {
		workspaceId: args.workspaceId,
	});
...
async function processRefundedDowngrade(
	...
	await ctx.runMutation(
		internal.billing.payments.applyWorkspaceSubscriptionQuantity,
		downgradeApplyArgs
	);
...
export const updateSubscriptionQuantity = action({
	...
	handler: async (
		...
		if (
			...
			await ctx.runMutation(internal.billing.payments.expirePendingUpgrade, {
				workspaceId,
			});
		}
		...
		const pendingPaymentRecoveryResult = async (message: string) => {
			...
			await ctx.runMutation(
				internal.billing.payments.recordWorkspacePendingPlanPayment,
				{
					workspaceId,
					subscriptionId: dodoSubscriptionId,
					planName,
		...
		try {
			...
			if (errorCode === "PLAN_CHANGE_NOT_ALLOWED_FOR_SCHEDULED_CANCELLATION") {
				...
				await ctx.runMutation(
					internal.billing.payments.clearWorkspaceScheduledCancellation,
					{
						workspaceId,
...
export const cancelSubscription = action({
	...
	handler: async (ctx, { workspaceId }) => {
		...
		await ctx.runMutation(
			internal.billing.payments.applyWorkspaceFreePlan,
			freePlanArgs
		);
