...
export const sendDirectMessageEmail = internalAction({
	args: {
	...
	handler: async (ctx, args): Promise<EmailNotificationResult> => {
		try {
			...
			const conversation = await ctx.runQuery(
				api.messaging.conversations._getConversationById,
				{
			...
			const sender = await ctx.runQuery(api.workspace.members.getMemberById, {
...
export const sendThreadReplyEmail = internalAction({
	args: {
		messageId: v.id("messages"),
	...
	handler: async (ctx, args): Promise<EmailNotificationResult> => {
		try {
			...
			// Get the replier
			const replier = await ctx.runQuery(api.workspace.members.getMemberById, {
...
export const sendWeeklyDigestEmails = internalAction({
	args: {
		dayOfWeek: v.string(),
	},
				...
				try {
					...
					if (
						...
						const result = await ctx.runAction(
							internal.notify.emailActions.sendWeeklyDigestEmail,
							{
								userId: user.userId,
								email: user.email,
								name: user.name,
	...
	handler: async (
		...
		try {
			...
			const card: CardDetails | null = await ctx.runQuery(
				api.board.board._getCardDetails,
				{ cardId }
			);
			if (!card) {
			...
			const assigneeEmail: string | null = await ctx.runQuery(
				api.board.board._getMemberEmail,
				{
					memberId: assigneeId,
				}
			);
			...
			const assigneeName: string | null = await ctx.runQuery(
				api.board.board._getMemberName,
				{
					memberId: assigneeId,
				}
			);
...
export const sendIssueAssignmentEmail = internalAction({
	args: {
		assigneeId: v.id("members"),
		issueId: v.id("issues"),
		assignerId: v.id("members"),
