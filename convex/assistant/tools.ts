...
export const searchNotes = query({
	...
	handler: async (ctx, args) => {
		await requireWorkspaceMember(ctx, args.workspaceId);
		const noteResults = (await ctx.runQuery(api.search.search.searchNotes, {
...
export const searchTasks = query({
	...
	handler: async (ctx, args) => {
		const { authUserId } = await requireWorkspaceMember(ctx, args.workspaceId);
		const taskResults = (await ctx.runQuery(api.search.search.searchTasks, {
			workspaceId: args.workspaceId,
			query: args.query,
			limit: args.limit ?? 12,
		})) as Array<{
...
export const getChannelSummary = query({
	...
	handler: async (
		...
		const debug = await ctx.runQuery(api.assistant.tools.getChannelDebug, {
			workspaceId: args.workspaceId,
...
export const getWorkspaceGeneralSummary = query({
	...
	handler: async (
		...
		const [noteResult, recentMessages, channels] = await Promise.all([
			...
			ctx.runQuery(api.messaging.messages.getRecentWorkspaceChannelMessages, {
				workspaceId: args.workspaceId,
...
export const semanticSearch: ReturnType<typeof action> = action({
	...
	handler: async (
		...
		const [directSearchResult, semanticSearchResult] = await Promise.allSettled(
			[
				ctx.runQuery(api.search.search.searchAll, {
					workspaceId: args.workspaceId,
					query: args.query,
					limit: retrievalLimit,
				}) as Promise<DirectSearchAllResults>,
				ctx.runAction(api.search.ragchat.semanticSearch, {
					workspaceId: args.workspaceId,
					query: args.query,
					limit: retrievalLimit,
				}) as Promise<{
