...
export const autoIndexMessage = action({
	...
	handler: async (ctx, args) => {
		const message = await ctx.runQuery(api.messaging.messages.getById, {
			id: args.messageId,
		});
		if (message) {
			const messageText = extractTextFromRichText(message.body);
			await ctx.runAction(api.search.ragchat.indexContent, {
...
export const autoIndexNote = action({
	...
	handler: async (ctx, args) => {
		try {
			const note = await ctx.runQuery(internal.content.notes._getNoteById, {
				noteId: args.noteId,
			});
			if (note) {
...
export const autoIndexTask = action({
	args: { taskId: v.id("tasks") },
	handler: async (ctx, args) => {
		const task = await ctx.runQuery(api.search.ragchat.getTaskForIndexing, {
			taskId: args.taskId,
		});
		if (task) {
			await ctx.runAction(api.search.ragchat.indexContent, {
...
export const autoIndexCard = action({
	args: { cardId: v.id("cards") },
	handler: async (ctx, args) => {
		const result = await ctx.runQuery(api.search.ragchat.getCardForIndexing, {
			cardId: args.cardId,
		});
		if (result) {
...
export const autoIndexCalendarEvent = action({
	...
	handler: async (ctx, args) => {
		const event = await ctx.runQuery(api.planning.calendar.getEventById, {
...
export const bulkIndexWorkspace = action({
	...
	handler: async (
		...
		const messages: Doc<"messages">[] = await ctx.runQuery(
			api.search.ragchat.getWorkspaceMessages,
			{
		...
		const notes: Doc<"notes">[] = await ctx.runQuery(
			api.search.ragchat.getWorkspaceNotes,
			{
				workspaceId: args.workspaceId,
				limit,
			}
		...
		const tasks: Doc<"tasks">[] = await ctx.runQuery(
			api.search.ragchat.getWorkspaceTasks,
			{
				workspaceId: args.workspaceId,
				limit,
			}
		...
		const cardsWithInfo: Array<{
			...
			list: Doc<"lists">;
			channel: Doc<"channels">;
		}> = await ctx.runQuery(api.search.ragchat.getWorkspaceCards, {
			workspaceId: args.workspaceId,
			limit,
		});
		...
		const events: Doc<"events">[] = await ctx.runQuery(
			api.planning.calendar.getWorkspaceEvents,
			{
				workspaceId: args.workspaceId,
				limit,
			}
		...
		for (const event of events) {
			try {
				await ctx.runAction(api.search.ragchat.autoIndexCalendarEvent, {
					eventId: event._id,
				});
			} catch {
				// Skip failed items
...
export const triggerBulkIndexing = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.scheduler.runAfter(0, api.search.ragchat.bulkIndexWorkspace, {
			workspaceId: args.workspaceId,
...
export const semanticSearch = action({
	...
	handler: async (ctx, args) => {
		...
		if (userId) {
			const member = await ctx.runQuery(api.workspace.members.current, {
				workspaceId: args.workspaceId,
			});
			if (!member) throw new Error("User is not a member of this workspace");

			const channels = await ctx.runQuery(api.messaging.channels.get, {
				workspaceId: args.workspaceId,
			});
		...
		try {
			...
			if (errorMessage.includes("No compatible namespace found")) {
				try {
					await ctx.runMutation(api.search.ragchat.autoInitializeWorkspace, {
						workspaceId: args.workspaceId,
						limit: 1000,
					});
				} catch {
