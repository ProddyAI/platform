...
export const getJoinCodeForVerification = action({
	...
	handler: async (ctx, args): Promise<string | undefined> => {
		...
		const currentMember = await ctx.runQuery(
			internal.workspace.invites.getMemberRole,
		...
		if (
			...
			return await ctx.runQuery(
				internal.workspace.invites.getWorkspaceJoinCodeForInviteVerification,
				{ workspaceId: args.workspaceId }
			);
		}

		// Step 3: If not a member, check if user has a valid pending invite
		const user = await ctx.runQuery(internal.workspace.invites.getUserById, {
			userId,
		});
		if (!user?.email) {
		...
		const pendingInvite = await ctx.runQuery(
			internal.workspace.invites.getPendingInviteForUser,
