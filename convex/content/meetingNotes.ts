...
export const generateAIInsights = action({
	...
	handler: async (ctx, args) => {
		if (!args.transcript || args.transcript.trim().length < 10) {
			await ctx.runMutation(internal.content.meetingNotes.updateStatus, {
				noteId: args.noteId,
				status: "failed",
			});
			return;
		...
		try {
			const note = await ctx.runQuery(api.content.meetingNotes.getById, {
				noteId: args.noteId,
			});
			...
			const existingGenerations = await ctx.runQuery(
				api.content.meetingNotes.getGenerations,
				{ roomId: note.roomId }
			);
...
${newTranscript}`;
			...
			if (object) {
				await ctx.runMutation(internal.content.meetingNotes.saveGeneration, {
