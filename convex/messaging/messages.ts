...
export const create = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.scheduler.runAfter(
			0,
			internal.billing.usageTracking.recordMessageCreated,
			{
				userId,
				workspaceId: args.workspaceId,
			}
		...
		if (args.parentMessageId) {
			await ctx.scheduler.runAfter(
				0,
				internal.notify.emailActions.sendThreadReplyEmail,
				{
					messageId,
					parentMessageId: args.parentMessageId,
				}
			...
			if (parentMessage) {
				...
				if (parentAuthor?.userId && parentAuthor.userId !== userId) {
					await ctx.scheduler.runAfter(
						2000,
						internal.notify.notifications.sendPushNotification,
						{
		...
		if (args.conversationId) {
			await ctx.scheduler.runAfter(
				0,
				internal.notify.emailActions.sendDirectMessageEmail,
				{
					messageId,
				}
			);
		...
		try {
			...
			for (const mentionedMemberId of Array.from(mentionedMemberIds)) {
				...
				await ctx.scheduler.runAfter(
					0,
					internal.notify.emailActions.sendMentionEmail,
					{
						mentionId,
					}
				);
		...
		await ctx.scheduler.runAfter(0, api.search.ragchat.autoIndexMessage, {
			messageId,
		});
