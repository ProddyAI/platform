...
export const AdBlockerProvider = ({ children }: { children: React.ReactNode }) => {
	...
	useEffect(() => {
		...
		const detectAdBlocker = async () => {
			try {
				...
				if (!cancelled) {
					// Default to assuming no blocker on unexpected errors
					setIsAdBlockerActive(false);
					setIsLoading(false);
				}
			}
 ...
 */
export const useAdBlockerDetectionContext = (): boolean => {
	const context = useContext(AdBlockerContext);
	if (!context) {
		throw new Error("useAdBlockerDetectionContext must be used inside AdBlockerProvider");
	}
	return context.isAdBlockerActive;
};
