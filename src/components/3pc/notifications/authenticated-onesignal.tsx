...
export const AuthenticatedOneSignalTracking = () => {
	...
	if (typeof window !== "undefined") {
		...
		console.log("🔍 [AuthenticatedOneSignalTracking] currentUser exists:", !!currentUser);
		if (currentUser) {
			console.log("🔥 [AuthenticatedOneSignalTracking] currentUser._id:", currentUser._id);
		}
	}

	// Don't render OneSignal until we know the auth state
	if (isLoading) {
		console.log("⏳ [AuthenticatedOneSignalTracking] Auth state loading, not rendering OneSignal yet");
		return null;
	}

	// Only render OneSignal for authenticated users
	if (!currentUser) {
		console.log("❌ [AuthenticatedOneSignalTracking] No currentUser, not rendering OneSignal");
		return null;
	}

	// Safety check: ensure userId exists
	if (!currentUser._id) {
		console.error("❌ [AuthenticatedOneSignalTracking] currentUser exists but _id is undefined!");
		return null;
	}

	console.log("✅ [AuthenticatedOneSignalTracking] Rendering OneSignal with userId:", currentUser._id);
