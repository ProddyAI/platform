...
export const UnifiedMessage = ({ data }: UnifiedMessageProps) => {
	...
	return (
		<Card
			...
			className="w-full max-w-sm !bg-gradient-to-r !from-slate-50 !to-slate-100 dark:!from-slate-800 dark:!to-slate-700 !text-gray-900 dark:!text-gray-50 shadow-lg border border-primary/20 dark:border-slate-500 hover:shadow-xl transition-shadow"
		>
			<div className="flex items-center justify-between p-3 min-h-[60px] gap-2">
				<div className="flex items-center space-x-2 flex-1 min-w-0">
					<div className="p-1.5 bg-primary/15 dark:bg-purple-500/50 rounded-lg flex-shrink-0">
						<Icon className="h-4 w-4 text-primary dark:text-purple-300" />
					</div>
					<div className="flex-1 min-w-0">
						<CardTitle className="text-sm font-semibold !text-gray-900 dark:!text-gray-50 truncate">
							{getTitle()}
						</CardTitle>

						{/* Show participants for live sessions */}
						{isLive && (
							<div className="flex items-center text-xs !text-gray-600 dark:!text-gray-200 mt-0.5">
								<Users className="h-3 w-3 mr-1 flex-shrink-0 dark:text-gray-200" />
