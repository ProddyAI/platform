...
export async function convertMermaidToExcalidrawScene(mermaidCode: string): Promise<{
  elements: any[];
  files: unknown;
}> {
  ...
  const { elements, files } = await (mermaidToExcalidraw as any).parseMermaidToExcalidraw(code, {
    fontSize: 20,
  });

  const excalidrawElements = (excalidraw as any).convertToExcalidrawElements(elements ?? null, {
    regenerateIds: true,
  });
