...
import { convertMermaidToExcalidrawScene, normalizeMermaidCode } from "@/features/canvas/diagram-ai/mermaid";
...
const STICKY_NOTE_MIN_HEIGHT = 80;
...
function isElementNewer(incoming: any, existing: any) {
  ...
  if (typeof incoming?.updated === "number" && typeof existing?.updated === "number") {
    if (incoming.updated !== existing.updated) return incoming.updated > existing.updated;
  }
...
function scenePointToViewport(point: { x: number; y: number }, appState: any) {
...
export const ExcalidrawCanvas = () => {
  ...
  const [excalidrawLibVersion, setExcalidrawLibVersion] = useState(0);
  const isApplyingRemoteSceneRef = useRef(false);
  const suppressBroadcastRef = useRef(false);
  const lastBroadcastedByIdRef = useRef(
    new Map<string, { version?: number; versionNonce?: number; isDeleted?: boolean }>()
  );
  ...
  useEffect(() => {
    const nextVersion = typeof (storedScene as any)?.version === "number" ? (storedScene as any).version : 0;
  ...
  const normalizeExcalidrawStorage = useMutation(({ storage }) => {
    ...
    const migrated: PersistedScene = {
      elements: Array.isArray((raw as any).elements) ? (raw as any).elements : [],
      appState: {
        ...DEFAULT_APP_STATE,
        ...sanitizeAppState((raw as any).appState),
      },
      files:
        (raw as any).files && typeof (raw as any).files === "object" ? (raw as any).files : {},
      version: typeof (raw as any).version === "number" ? (raw as any).version : 1,
    };
  ...
  const generateDiagramFromPrompt = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const message = body?.error || body?.message || "Diagram generation failed";
      ...
      const { elements: newElements, files } = await convertMermaidToExcalidrawScene(mermaid);
      ...
      const getCommonBounds = excalidrawLibRef.current?.getCommonBounds as
        ...
        | undefined;
      const [x1, y1, x2, y2] = (getCommonBounds || getCommonBoundsFallback)(newElements as any);
  ...
  useEffect(() => {
    ...
    const version = typeof (storedScene as any).version === "number" ? (storedScene as any).version : 0;
    ...
    const nextScene = {
      elements: Array.isArray((storedScene as any).elements) ? (storedScene as any).elements : [],
      appState: {
        ...DEFAULT_APP_STATE,
  ...
  useEventListener(({ event }) => {
    ...
    const incomingElements = Array.isArray((event as any).elements) ? (event as any).elements : [];
  ...
  const insertStickyNote = async () => {
    ...
    const convertToExcalidrawElements = excalidrawLibRef.current?.convertToExcalidrawElements as any;
    const restoreElements = excalidrawLibRef.current?.restoreElements as any;
    const newElements = (restoreElements && convertToExcalidrawElements)
      ? restoreElements(
          convertToExcalidrawElements([shadowSkeleton, rectSkeleton, textSkeleton]) as any,
          null
        )
    ...
    api.updateScene({
      ...
      commitToHistory: true,
      appState: {
        selectedElementIds: Object.keys(selection).length ? selection : { [noteId]: true },
      },
    } as any);
    ...
    <div className="h-full w-full" ref={excalidrawHostRef}>
      ...
      <Excalidraw
        ...
        onPointerUpdate={({ pointer, button }: any) => {
          ...
          const zoom =
            typeof appState?.zoom?.value === "number"
              ...
              : typeof appState?.zoom === "number"
                ? appState.zoom
                : 1;
          const width = typeof appState?.width === "number" ? appState.width : 0;
          const height = typeof appState?.height === "number" ? appState.height : 0;
          ...
          const sceneX = isLikelyViewport ? pointer.x / zoom - scrollX : pointer.x;
          const sceneY = isLikelyViewport ? pointer.y / zoom - scrollY : pointer.y;
          const viewportX = isLikelyViewport ? pointer.x : (pointer.x + scrollX) * zoom;
          const viewportY = isLikelyViewport ? pointer.y : (pointer.y + scrollY) * zoom;
        ...
        onChange={(elements: readonly any[], appState: any, files: any) => {
          ...
          const elementsArray = Array.isArray(elements) ? Array.from(elements) : [];
          ...
          if (selectedIds && typeof selectedIds === "object") {
            const selectedKeys = Object.keys(selectedIds).filter((k) => (selectedIds as any)[k]);
            const selectedShadow = selectedKeys
              .map((id) => elementsArray.find((el) => el?.id === id))
              .find((el) => getStickyNoteDecorType(el) === "sticky-note-shadow");

            if (selectedShadow) {
              const parentId = getStickyNoteParentId(selectedShadow);
              const parentNote = parentId ? elementsArray.find((el) => el?.id === parentId && isStickyNoteElement(el)) : null;
              if (parentNote) {
                const api = excalidrawApiRef.current;
                if (api) {
          ...
          for (const el of elementsArray) {
            ...
            if (shadow.locked) {
              const mutateElement = excalidrawLibRef.current?.mutateElement as any;
              if (mutateElement) {
          ...
          if (didSyncDecor) {
            ...
            if (api) {
              isSyncingStickyDecorRef.current = true;
              try {
                api.updateScene({ elements: elementsArray, commitToHistory: false } as any);
              } finally {
                window.setTimeout(() => {
                  isSyncingStickyDecorRef.current = false;
                }, 0);
          ...
          saveTimerRef.current = window.setTimeout(() => {
            const base = Math.max(currentVersionRef.current, lastLocalWriteVersionRef.current, Date.now() * 1000);
        ...
        {ExcalidrawSidebar ? (
          <ExcalidrawSidebar name="ai-format">
            ...
            <div className="flex flex-col gap-3 p-3">
              ...
              />
              <Button onClick={generateDiagramFromPrompt} disabled={isGenerating}>
